...
<div class="row g-4 mb-5">
            ...
            <div class="card h-100 shadow-sm border rounded-4 dashboard-card-simple">
                <div class="card-body p-4">
                    ...
                    <div class="d-flex align-items-baseline">
                        <span class="h3 fw-bold text-dark mb-0">{{ $jumlahDosen }}</span>
        ...
        <a href="{{ route('gedung.index') }}" class="text-decoration-none">
            <div class="card h-100 shadow-sm border rounded-4 dashboard-card-simple">
                <div class="card-body p-4">
                    ...
                    <div class="d-flex align-items-baseline">
                        <span class="h3 fw-bold text-dark mb-0">{{ $jumlahGedung }}</span>
        ...
        <a href="{{ route('ruangan.index') }}" class="text-decoration-none">
            <div class="card h-100 shadow-sm border rounded-4 dashboard-card-simple">
                <div class="card-body p-4">
                    ...
                    <div class="d-flex align-items-baseline">
                        <span class="h3 fw-bold text-dark mb-0">{{ $jumlahRuangan }}</span>
    ...
    <div class="col-3">
        <a href="{{ route('mata-kuliah.index') }}" class="text-decoration-none">
            <div class="card h-100 shadow-sm border rounded-4 dashboard-card-simple">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center mb-3">
                    ...
                    <div class="d-flex align-items-baseline flex-wrap">
                        <span class="h3 fw-bold text-dark mb-0">{{ $jumlahMataKuliah }}</span>
                        <span class="ms-1 text-muted small me-2">Matkul</span>
                        <span class="badge bg-light text-dark border fw-normal">{{ $jumlahKelas }} Kelas</span>
                    </div>
                </div>
            </div>
        </a>
        ...
        <div class="card border-0 shadow-sm rounded-4 bg-white border-top border-5 border-dark">
            <div class="card-body p-4 text-center">
                <div class="mx-auto" style="max-width: 600px;">
                    ...
                    <a href="{{ route('pengaturan.index') }}" class="btn btn-outline-dark rounded-pill px-5 shadow-sm transition-all hover-translate-y">
        ...
        <div class="card border shadow-sm rounded-4 overflow-hidden border-primary-subtle">
            <div class="card-body p-5 text-center bg-white">
                <div class="mx-auto" style="max-width: 800px;">
                    ...
                    <form action="{{ route('jadwal.buat') }}" method="POST" id="generateForm" class="m-0 mx-auto" style="max-width: 450px;">
                        @csrf
                    ...
                    <div class="mt-4 text-muted small">
                        <i class="bi bi-info-circle me-1"></i> Proses ini mungkin memakan waktu beberapa detik. Lihat <a href="{{ route('dokumentasi.index') }}" class="text-primary text-decoration-none fw-bold">Dokumentasi</a> untuk panduan lebih lanjut.
                    </div>
                </div>
            </div>
        </div>
...
<div id="scheduleResults" class="card border shadow-sm rounded-4 overflow-hidden">
    <div class="card-header bg-white py-3 px-4 d-flex justify-content-between align-items-center border-bottom">
        ...
        <div class="d-flex align-items-center gap-2">
            @if(count($jadwal) > 0)
                ...
                <form action="{{ route('jadwal.bersihkan') }}" method="POST" id="resetForm">
                    @csrf
                    @method('DELETE')
    ...
    <div class="card-body p-4">
        @php
            $groupedSchedules = $jadwal->groupBy(function($s) {
                return $s->kelas?->mataKuliah?->semester ?? 'Lainnya';
            })->sortKeys();
            $globalRowIndex = 0;
        @endphp
            ...
            <div class="table-responsive border rounded-3 mb-2 shadow-sm">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody class="border-top-0">
                        @php
                            $sksDuration = \App\Models\Pengaturan::getValue('durasi_sks', 50);
                            $dayBadgeColors = [
                                'Senin' => 'primary',
                                'Selasa' => 'success',
                                'Rabu' => 'warning',
                        ...
                        @foreach($semesterSchedules as $s)
                            ...
                            <tr class="reveal-row" style="animation-delay: {{ $globalRowIndex * 0.05 }}s">
                                ...
                                <td>
                                    <div class="fw-bold small">{{ $s->kelas?->mataKuliah?->nama ?? '-' }}</div>
                                    <div class="x-small text-muted" style="font-size: 0.7rem;">{{ $sks }} SKS</div>
                                </td>
                                <td>
                                    <div class="small">{{ $s->kelas?->dosen?->nama ?? '-' }}</div>
                                </td>
                                <td class="px-4 text-center">
                                    <div class="badge bg-light text-dark border px-3 py-1 fw-medium" style="font-size: 0.75rem;">
                                        {{ $s->ruangan?->nama ?? '?' }}
                                    </div>
                                    <div class="x-small text-muted mt-1" style="font-size: 0.65rem;">{{ $s->ruangan?->gedung?->nama ?? '-' }}</div>
                                </td>
                            </tr>
                            @php $globalRowIndex++; @endphp
                        @endforeach
    ...
    function initSchedulePage() {
        ...
        if (generateForm) {
            generateForm.addEventListener('submit', function(e) {
                ...
                const logs = [
                    ...
                    "Checking Lecturer Workload...",
                    "Starting Evolution Cycle...",
                    "Generation 1... Fitness: 0.15",
                    "Generation 50... Fitness: 0.32",
                    "Generation 100... Fitness: 0.54",
                    "Generation 250... Fitness: 0.68",
                    "Generation 500... Fitness: 0.82",
                    "Stagnation detected. Boosting Mutation Rate...",
                    "Generation 800... Fitness: 0.94",
                    "Generation 1200... Fitness: 0.99",
                    "OPTIMAL SOLUTION FOUND! Fitness: 1.0",
        ...
        if (btnProcessExport) {
            btnProcessExport.addEventListener('click', function() {
                ...
                let baseUrl = format === 'csv' ? "{{ route('jadwal.ekspor.csv') }}" : "{{ route('jadwal.ekspor.pdf') }}";
