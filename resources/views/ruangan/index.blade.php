            ...
            <div class="card-body p-4">
                <form action="{{ route('ruangan.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="gedung_id" class="form-select bg-light" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama" class="form-control bg-light" placeholder="Contoh: R.301" required>
                    </div>
                    <div class="mb-4">
                        <label class="form-label fw-medium">Tipe Ruangan</label>
                        <div class="d-flex gap-3">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="tipe" id="typeTheory" value="teori" checked>
                                <label class="form-check-label" for="typeTheory">Teori</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="tipe" id="typeLab" value="praktikum">
                                ...
                                <td class="ps-4 py-3">
                                    <div class="d-flex align-items-center">
                                        <div class="bg-success-subtle text-success rounded-circle p-2 me-3" style="width: 40px; height: 40px; display: flex; align-items: center; justify-content: center;">
                                            <span class="fw-bold">{{ substr($r->nama, 0, 1) }}</span>
                                        </div>
                                        <div>
                                            <div class="fw-bold text-dark">{{ $r->nama }}</div>
                                            <div class="small text-muted">{{ $r->gedung->nama ?? '-' }}</div>
                                        </div>
                                    </div>
                                </td>
                                                ...
                                                <form action="{{ route('ruangan.update', $r->id) }}" method="POST">
                                                    ...
                                                    <div class="modal-body p-4 text-start">
                                                        <div class="mb-3">
                                                            ...
                                                            <select name="gedung_id" class="form-select bg-light" required>
                                                                @foreach($gedung as $g)
                                                                    <option value="{{ $g->id }}" {{ $r->gedung_id == $g->id ? 'selected' : '' }}>{{ $g->nama }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="mb-3">
                                                            <label class="form-label fw-medium">Nama Ruangan</label>
                                                            <input type="text" name="nama" class="form-control bg-light" value="{{ $r->nama }}" required>
                                                        </div>
                                                        <div class="mb-0">
                                                            <label class="form-label fw-medium">Tipe</label>
                                                            <select name="tipe" class="form-select bg-light" required>
                                                                <option value="teori" {{ $r->tipe == 'teori' ? 'selected' : '' }}>Teori</option>
                                                                <option value="praktikum" {{ $r->tipe == 'praktikum' ? 'selected' : '' }}>Laboratorium</option>
                                                            </select>
                                                        </div>
                                                    </div>
