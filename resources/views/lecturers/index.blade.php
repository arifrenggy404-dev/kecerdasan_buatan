                ...
                <form action="{{ route('lecturers.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="name" class="form-control border-start-0 bg-light" placeholder="Contoh: Dr. Budi Santoso" required>
                        </div>
                    </div>
                            ...
                            <tr>
                                <td class="ps-4 py-3">
                                    <div class="d-flex align-items-center">
                                        <div class="bg-primary-subtle text-primary rounded-circle p-2 me-3" style="width: 40px; height: 40px; display: flex; align-items: center; justify-content: center;">
                                            <span class="fw-bold">{{ substr($l->name, 0, 1) }}</span>
                                        </div>
                                ...
                                <td class="pe-4 py-3 text-end">
                                    <div class="d-flex justify-content-end gap-2">
                                        <button class="btn btn-sm btn-outline-primary rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#editModal{{ $l->id }}">Edit</button>
                                        <form action="{{ route('lecturers.destroy', $l->id) }}" method="POST" onsubmit="return confirm('Hapus dosen ini?')">
                                            @csrf @method('DELETE')
                                    ...
                                    <!-- Edit Modal -->
                                    <div class="modal fade" id="editModal{{ $l->id }}" tabindex="-1" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content border-0 shadow-lg rounded-4">
                                                <form action="{{ route('lecturers.update', $l->id) }}" method="POST">
                                                    @csrf @method('PUT')
                                                    ...
                                                    <div class="modal-body p-4 text-start">
                                                        <div class="mb-0">
                                                            <label class="form-label fw-medium">Nama Lengkap</label>
                                                            <input type="text" name="name" class="form-control bg-light" value="{{ $l->name }}" required>
                                                        </div>
                                                    </div>
